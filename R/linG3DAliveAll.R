...
#'
#' @details This code generates the 3D lineage tree of all clones including
...
if (toPrint==1){
  pathFigs <- paste0("fig_clonesAlive")
  dir.create(pathFigs)
}
...
cellID <- as.matrix(read.table(paste0(dataDirectory,"/cellID_",tmax,".txt"),header = F))
...
bgplot3d({ plot.new();
  title(paste0(main = "final 3D traces of survived clones = ",cloneNum), line = 0,cex.main=2.5)
  text(x=0.2,y=0.122,"iterations/time x 200",srt=-41,cex=3) })

if (toPrint==1){
  rgl.snapshot(paste0(pathFigs,"/tree_alive_combined",".png"), fmt = "png", top = TRUE)}
}  # end function
